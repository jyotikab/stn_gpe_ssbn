...
#--------------------------------------------------------------------------------------------
# This script reproduces Supp Figure S6, S7 in the manuscript, expects "psdbEffect_inh_ip_"+prefix1+str(gpe_ip)+"_exc_ip_"+str(stn_ip)+"_inh_ratio_"+str(gpe_rat)+"_exc_ratio_"+str(stn_rat)+"ref_"+str(int(float(ref))), files from simulations different proprtion of STn and GPe bursty neurons (gpe_rat:0-1.0, stn_rat:0-1.0) and at least 3 firing rate combinations:np.array([[500.0,1000.0 ],[900.0,1400.0],[1300.0,1400.0]])
#--------------------------------------------------------------------------------------------
...
if ref == "":
	...
	data = pickle.load(open("../rate_effect/Data_"+ref+".pickle","r"))
...
left0 = 0.18
hspace=0.05
gs0.update(left=left0,right=left0+gridWidth,top=top+0.28,bottom=top+2.5*hspace,wspace=0.05,hspace=hspace)
gs0Hands =[]
...
gs0Hands[0].text(170,1750,"A",fontweight='bold',fontsize=18)
boxcolor='black'
lw = 2.0
...
for gpe_ip,stn_ip in zip(poi_rate_bkg_gpe,poi_rate_bkg_stn) :
		for k,gpe_rat in enumerate(gpe_ratio_psdb):
			for l,stn_rat in enumerate(stn_ratio_psdb):
				prefix = "psdbEffect_inh_ip_"+str(gpe_ip)+"_exc_ip_"+str(stn_ip)+"_inh_ratio_"+str(gpe_rat)+"_exc_ratio_"+str(stn_rat)+"ref_"+str(int(float(ref)))
		...
		im5=gsHands[2].pcolor(stn_ratio_psdb,gpe_ratio_psdb,big_gpe_spec_lis[i],cmap=cm.BuGn,vmin=0.3,vmax=0.6)
		...
		img4=gsHands[3].pcolor(stn_ratio_psdb,gpe_ratio_psdb,big_stn_spec_lis[i],cmap=cm.BuGn,vmin=0.3,vmax=0.6)
		...
		if i == 2:
			cbar_ax3 = fig.add_axes([left3+gridWidth+0.01, 0.05, 0.01, top*0.4])
			cb1 = fig.colorbar(img4,cax=cbar_ax3)
			cb1.ax.set_yticklabels(cb1.ax.get_yticklabels(),fontsize=10,fontweight='bold')
		...
		img32 = gsHands[1].pcolor(stn_ratio_psdb,gpe_ratio_psdb,big_stn_fr_lis[i],cmap=cm.BuGn)
...
fig.savefig("figs/Fig5_"+str(ref)+"ms.pdf")
fig.savefig("figs/Fig5_"+str(ref)+"ms.png")
